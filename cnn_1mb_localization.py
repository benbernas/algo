...
load_path = dir_path + r'\algo\generate_database\database\database_IQ_mb_noised_sizeImg_91_128_1mb_10blocks_comp_coreULM2.h5'
...
with h5py.File(load_path, 'r') as file:
    norm_IQ_mb_noised = file["norm_IQ_mb_noised_shuffled"][:,:,:]
    mb_coord = file["simu_mb_coord"][:,:]
    # mb_coord = file["simu_mb_coord"][:,:,:]
    IQ_train = file["IQ_train"][:,:,:]
    # coord_train = file["coord_train"][:,:]
    coord_train = file["coord_train"][:,:,0]
    # xTrain = file["xTrain"][:,:,:]
    # yTrain = file["yTrain"][:,:]
    xTest = file["xTest"][:,:,:]
    # yTest = file["yTest"][:,:]
...
nblocks = int(norm_IQ_mb_noised.shape[0]/nframes)
...
save_folder = dir_path + r'\results\test_simu_1bulle\cnn_prediction\IQ_mb_no_noise\adding_gaussian_noise_5_per_correction\comp_coreULM'
save_subfolder0 = r'\teeest\nblocks_{}'.format(nblocks)
save_subfolder1 = r'\sizeConv2D_{}'.format(sizeConv2D_1)+'_{}'.format(sizeConv2D_2)
save_subfolder2 = r'\cross_val_{}'.format(crossVal) + '_{}'.format(nKFold) 
save_subfolder3 = r'\epoch_{}'.format(Epoch)
# save_subfolder5 = r'\nKFold_{}'.format(nKFold)
save_subfolder4 = r'\dropout_afterFlatten'
...
def create_model(input_shape):
    cnn = Sequential([
        Conv2D(16, (sizeConv2D_1, sizeConv2D_1), activation=activationConv2D, input_shape=input_shape),
        MaxPooling2D((sizeMaxPool, sizeMaxPool)),
        Conv2D(32, (sizeConv2D_2, sizeConv2D_2), activation=activationConv2D),
        # Conv2D(32, (sizeConv2D_2, sizeConv2D_2), activation=activationConv2D, padding='same'),
        MaxPooling2D((sizeMaxPool, sizeMaxPool)),
        # Conv2D(64, (sizeConv2D_2, sizeConv2D_2), activation=activationConv2D),
        # MaxPooling2D((sizeMaxPool, sizeMaxPool)),
        Flatten(),
        Dropout(dropoutValue),
        # Dense(128, activation=activationDenseLayer),
...
print(' -> RMSE between true position and predicted position of MBs = ', "[{:.4f}, ".format(avg_rmse[0]), "{:.4f}]".format(std_rmse[0]), 'mm')
print(' -> RMSE over x between true position and predicted position of MBs = ', "[{:.4f}, ".format(avg_rmse[1]), "{:.4f}]".format(std_rmse[1]), 'mm')
print(' -> RMSE over z between true position and predicted position of MBs = ', "[{:.4f}, ".format(avg_rmse[2]), "{:.4f}]".format(std_rmse[2]), 'mm')
#%% Show figs
...
if save_model == 1:
    ...
    dump({'img_depth': img_depth,
          'img_width': img_width,
          'sizeConv2Dlayer1': sizeConv2D_1,
          'sizeConv2Dlayer2': sizeConv2D_2,
